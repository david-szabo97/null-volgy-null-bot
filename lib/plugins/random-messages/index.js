...
const { getRandomArrayItem } = require('../random-array')
  ...
  client.on('message', msg => {
    for (const word of triggerWords) {
      if (msg.content.toLowerCase().includes(word)) {
        return msg.reply(getRandomArrayItem(knownMessageReplies))
      }
    }
    msg.reply(getRandomArrayItem(unknownMessageReplies))
  })
}
